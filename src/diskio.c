...
)
{
  ...
  for (n = 10; n; n--)
    MICROSD_XferSpi(0xff);   /* 80 dummy clocks */

  ty = 0;
  if (MICROSD_SendCmd(CMD0, 0) == 1)
  {          /* Enter Idle state */
    MICROSD_TimeOutSet(1000);                   /* Initialization timeout of 1000 msec */
    if (MICROSD_SendCmd(CMD8, 0x1AA) == 1)
    {    /* SDv2? */
      for (n = 0; n < 4; n++)
        ocr[n] = MICROSD_XferSpi(0xff); /* Get trailing return value of R7 resp */
      ...
      {   /* The card can work at vdd range of 2.7-3.6V */
        ...
        if (!MICROSD_TimeOutElapsed() && MICROSD_SendCmd(CMD58, 0) == 0)
